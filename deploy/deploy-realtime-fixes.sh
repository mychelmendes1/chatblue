#!/usr/bin/expect -f

set timeout 300
set server "84.247.191.105"
set user "root"
set password "fjykwePMThmj6nav"
set local_base "/Users/mychel/Downloads/Projetos/chatblue/chatblue"

# Copy updated files
spawn scp -o StrictHostKeyChecking=no \
    ${local_base}/apps/web/lib/socket.ts \
    ${local_base}/apps/web/components/providers/socket-provider.tsx \
    ${local_base}/apps/web/components/chat/chat-window.tsx \
    ${local_base}/apps/api/src/sockets/index.ts \
    ${local_base}/apps/api/src/services/message-processor.service.ts \
    ${user}@${server}:/tmp/

expect {
    "password:" {
        send "${password}\r"
    }
    "yes/no" {
        send "yes\r"
        expect "password:"
        send "${password}\r"
    }
}

expect eof

# SSH and deploy
spawn ssh -o StrictHostKeyChecking=no ${user}@${server}

expect {
    "password:" {
        send "${password}\r"
    }
    "yes/no" {
        send "yes\r"
        expect "password:"
        send "${password}\r"
    }
}

expect "# "

send "echo '=== Movendo arquivos corrigidos ==='\r"
expect "# "

send "cp /tmp/socket.ts /opt/chatblue/app/apps/web/lib/socket.ts\r"
expect "# "
send "cp /tmp/socket-provider.tsx /opt/chatblue/app/apps/web/components/providers/socket-provider.tsx\r"
expect "# "
send "cp /tmp/chat-window.tsx /opt/chatblue/app/apps/web/components/chat/chat-window.tsx\r"
expect "# "
send "cp /tmp/index.ts /opt/chatblue/app/apps/api/src/sockets/index.ts\r"
expect "# "
send "cp /tmp/message-processor.service.ts /opt/chatblue/app/apps/api/src/services/message-processor.service.ts\r"
expect "# "

send "cd /opt/chatblue/app/apps/api\r"
expect "# "

send "echo '=== Fazendo build da API ==='\r"
expect "# "
send "pnpm build 2>&1 | tail -30\r"
expect "# "

send "cd /opt/chatblue/app/apps/web\r"
expect "# "

send "echo '=== Fazendo build do Web ==='\r"
expect "# "
send "pnpm build 2>&1 | tail -30\r"
expect "# "

send "echo '=== Reiniciando serviços ==='\r"
expect "# "
send "pm2 reload chatblue-api --update-env\r"
expect "# "
send "pm2 reload chatblue-web --update-env\r"
expect "# "

send "sleep 5\r"
expect "# "

send "pm2 status\r"
expect "# "

send "pm2 logs chatblue-api --lines 10 --nostream | tail -10\r"
expect "# "

send "echo '=== Verificando saúde dos serviços ==='\r"
expect "# "
send "curl -s -o /dev/null -w '%{http_code}' http://localhost:3001/health && echo ' - API OK!' || echo 'API ainda não responde'\r"
expect "# "
send "curl -s -o /dev/null -w '%{http_code}' http://localhost:3000/health && echo ' - Web OK!' || echo 'Web ainda não responde'\r"
expect "# "

send "exit\r"
expect eof








