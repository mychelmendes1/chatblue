...
/**
 * Service for RAG operations on codebase
 * Uses the KnowledgeContext system with a special "system-code" context
 */
export class CodeRAGService {
  ...
  async findRelevantCode(
    ...
    try {
      // Find or create the "system-code" context for this company
      ...
      if (!codeContext) {
        logger.info(`No system-code context found for company ${companyId}`);
        return { chunks: [], context: null };
      }

      // Use semantic search if embeddings are available
      if (aiApiKey && aiProvider && codeContext.sources.length > 0) {
        try {
          ...
          // Get all chunks from code sources
          const allChunks = codeContext.sources.flatMap((source) =>
          ...
          logger.warn('Error in semantic search for code, falling back to keyword search:', error);
          // Fallback to keyword search
          return this.keywordSearchCode(codeContext, query, limit);
        }
      }
      ...
      logger.error('Error finding relevant code:', error);
      return { chunks: [], context: null };
    }
  }

  /**
   * Keyword-based search in code
   */
  private keywordSearchCode(
    context: any,
    query: string,
    limit: number
  ): {
  ...
  /**
   * Get code context as formatted string
   */
  async getCodeContext(
    ...
    const { chunks, context } = await this.findRelevantCode(
      companyId,
      query,
      aiProvider,
      aiApiKey,
    ...
    // Format as readable code context
    const sections: string[] = [];
    for (const [sourceName, sourceChunks] of chunksBySource.entries()) {
      const codeBlocks = sourceChunks
        .map((chunk) => chunk.content)
        .join('\n\n---\n\n');
      sections.push(`[Arquivo: ${sourceName}]\n\n${codeBlocks}`);
    }
